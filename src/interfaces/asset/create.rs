...
use crate::generated::schema::{ModuleType, ActionData, DataItemValue, DataItem, Creator, ModuleData};
        ...
        if let ActionData::CreateAssetV1 {
            ...
            if rng.len() > 0 {
                ...
                if creator_shares.len() != rng.len() {
                    return Err(DigitalAssetProtocolError::ActionError("Creators and Creator Shares must match".to_string()))
                }
                let mut i = 0;
                for c in rng {
                    let creator = accounts.prepare_account(c as usize, "creator", Constraints::read_only_optional_signer())?;
